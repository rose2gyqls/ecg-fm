"""
Phase 3 (v6 / MoRyECG + GlobalRefineBlock): Masked beat modeling with physiology-factorized
Transformer + STFT-conditioned [GLOB] + SimCLR contrastive auxiliary loss.

Differences vs training/pretrain/train.py (v4):
  - Model: MoRyECGv6FoundationModel  (v5 + final GlobalRefineBlock; 4D H + separate g, intra-beat/inter-beat
    factorized blocks, RR-aware shared additive bias)
  - STFT leakage prevention: lead-dropout zero  +  beat-aligned STFT-time
    bin zero (uses dataset's new beat_t_starts / beat_t_ends keys)
  - Per-record beat_valid_mask propagated everywhere (MLM loss, rhythm-attn
    key_padding_mask, padded-slot zeroing)
  - RR / Fid auxiliary heads kept (gated by weight = 0 in default v5 config)
  - ckpt['model_cfg']['arch'] = 'moryecg_v6'  ← used by benchmark adapter

Single GPU:
    python -m training.pretrain.train_v6 --config configs/pretrain/<...>_v5.yaml

Multi-GPU:
    CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 \
        -m training.pretrain.train_v6 --config <...>_v5.yaml
"""
...
from models.transformer.moryecg_v6_model import MoRyECGv6FoundationModel
from models.heads.mlm_head import (
    MaskedBeatModelingHead, MaskedRhythmHead, MaskedFiducialHead,
)
from models.heads.contrastive_head import ProjectionHead, nt_xent_loss
from training.pretrain.masking import (
    apply_masking, lead_dropout_schedule, mask_ratio_schedule,
)
...
# ──────────────────────────────────────────────────────────────────────────────
# STFT leakage prevention
# ──────────────────────────────────────────────────────────────────────────────
def stft_time_mask_from_beat_mask(
    beat_mask: torch.Tensor,           # (B, N, L) bool
    beat_t_starts: torch.Tensor,       # (B, N) long
    beat_t_ends: torch.Tensor,         # (B, N) long
    T_stft: int,
) -> torch.Tensor:
    """Bool (B, T_stft) — True = STFT time bin must be zeroed.

    Collapse beat_mask to (B, N) via .any(-1): if ANY lead at beat i is
    masked, zero out every STFT frame whose support overlaps beat i. This
    is conservative under the iid masking strategy (which can mask a single
    lead) and exact under the cross-lead-aligned strategy (the default).
    Padding beats carry start == end == 0, contributing an empty span.
    """
    B, N, _ = beat_mask.shape
    bm_bn = beat_mask.any(dim=-1)                                       # (B, N)
    t_idx = torch.arange(T_stft, device=beat_mask.device)
    in_span = (t_idx[None, None, :] >= beat_t_starts[:, :, None]) & \
              (t_idx[None, None, :] <  beat_t_ends[:, :, None])
    in_span = in_span & bm_bn[:, :, None]
    return in_span.any(dim=1)                                           # (B, T_stft)


def apply_stft_leakage_mask(
    stft: torch.Tensor,                # (B, L, F, T')
    lead_mask: torch.Tensor,           # (B, L) bool — True = lead dropped
    beat_mask: torch.Tensor,           # (B, N, L) bool — True = beat masked
    beat_t_starts: torch.Tensor,       # (B, N) long
    beat_t_ends: torch.Tensor,         # (B, N) long
) -> torch.Tensor:
    """Zero out (a) dropped-lead channels and (b) STFT time bins covering
    any masked beat. Combined, this guarantees the [GLOB] STFT input shares
    no information with the masked morphology codes the model is asked to
    recover."""
    B, L, _F, T_stft = stft.shape
    out = stft * (~lead_mask).to(stft.dtype).view(B, L, 1, 1)
    t_mask = stft_time_mask_from_beat_mask(beat_mask, beat_t_starts, beat_t_ends, T_stft)
    out = out * (~t_mask).to(stft.dtype).view(B, 1, 1, T_stft)
    return out
...
def train(cfg: dict, resume: str | None = None):
    ...
    # ── Throughput knobs (no behavior change) ───────────────────────────────
    # cuDNN.benchmark picks the fastest conv algo for the (fixed) STFT shape.
    # TF32 matmul on Ampere+ gives free speed for fp32 ops outside autocast.
    if torch.cuda.is_available():
        torch.backends.cudnn.benchmark = True
    torch.set_float32_matmul_precision("high")

    # ── Mixed precision (bf16 default on H100/H200) ─────────────────────────
    amp_cfg     = (cfg.get("training", {}) or {}).get("amp", {}) or {}
    amp_enabled = bool(amp_cfg.get("enabled", True))
    amp_dtype_str = str(amp_cfg.get("dtype", "bfloat16")).lower()
    amp_dtype = torch.bfloat16 if amp_dtype_str in ("bf16", "bfloat16") else torch.float16
    # bfloat16 has fp32 dynamic range → no GradScaler needed.
    # fp16 would require GradScaler; we stick to bf16 on H200/H100 by default.
    if amp_enabled and amp_dtype is torch.float16:
        # Defensive: float16 path needs a GradScaler, which we don't wire up
        # here. Fail loudly rather than silently overflow.
        raise NotImplementedError(
            "amp dtype=float16 not supported in train_v5 (use bfloat16 on H200/H100)"
        )

    if is_main:
        print(f"[Pretrain v6] DDP={ddp}  world_size={world_size}  device={device}  "
              f"amp={amp_enabled}({amp_dtype_str})  tf32=on  cudnn.benchmark=on")
    ...
    if is_main:
        print(f"[Pretrain v6] Tokenizer loaded from {tok_ckpt_path} and frozen.")

    # ── Build model_cfg (persisted into ckpt for adapter) ───────────────────
    model_cfg = dict(cfg["model"])
    model_cfg["arch"]             = "moryecg_v6"
    model_cfg["codebook_size"]    = int(cfg["tokenizer"]["codebook_size"])
    model_cfg["n_leads"]          = int(cfg["data"].get("n_leads", 12))
    model_cfg["max_beats"]        = int(cfg["data"].get("max_beats_per_lead", 30))
    ...
    if rr_mean is not None and rr_std is not None:
        ...
        if is_main:
            print(f"[Pretrain v6] RhythmMLP normalize: mean={ctx['rhythm_mean']} std={ctx['rhythm_std']}")

    # ── Loss weight introspection (controls which heads are even built) ────
    # v5 default: morph=1, rhythm=0, fid=0, contrastive=0.3
    # We instantiate (and DDP-wrap) ONLY the heads that will actually be
    # called in the forward path. This lets DDP use static graphs / no
    # find_unused_parameters scan, which on H200 is a real win.
    _loss_cfg = (cfg.get("training", {}) or {}).get("loss", {}) or {}
    _ctr_cfg  = (cfg.get("training", {}) or {}).get("contrastive", {}) or {}
    _morph_w_init  = float(_loss_cfg.get("morphology_weight", 1.0))
    _rhythm_w_init = float(_loss_cfg.get("rhythm_weight", 0.0))
    _fid_w_init    = float(_loss_cfg.get("fiducial_weight", 0.0))
    _ctr_w_init    = float(_ctr_cfg.get("weight", 0.0))

    # ── Model + heads ───────────────────────────────────────────────────────
    model = MoRyECGv6FoundationModel(model_cfg).to(device)
    mlm_head = MaskedBeatModelingHead(
        d_model=int(cfg["model"]["d_model"]),
        codebook_size=int(cfg["tokenizer"]["codebook_size"]),
    ).to(device) if _morph_w_init > 0 else None

    rr_head = (MaskedRhythmHead(d_model=int(cfg["model"]["d_model"])).to(device)
               if _rhythm_w_init > 0 else None)
    fid_head = (MaskedFiducialHead(d_model=int(cfg["model"]["d_model"])).to(device)
                if _fid_w_init > 0 else None)

    ctr_proj_hidden = int(_ctr_cfg.get("proj_hidden", int(cfg["model"]["d_model"])))
    ctr_proj_out    = int(_ctr_cfg.get("proj_out", 128))
    ctr_temperature = float(_ctr_cfg.get("temperature", 0.1))
    ctr_w_max       = _ctr_w_init
    ctr_warmup      = int(_ctr_cfg.get("warmup_epochs", 0))
    proj_head = (ProjectionHead(
        d_in=int(cfg["model"]["d_model"]),
        d_hidden=ctr_proj_hidden,
        d_out=ctr_proj_out,
    ).to(device) if ctr_w_max > 0 else None)

    if is_main:
        print(f"[Pretrain v6] heads built: "
              f"mlm={'on' if mlm_head is not None else 'off'}  "
              f"rr={'on' if rr_head is not None else 'off'}  "
              f"fid={'on' if fid_head is not None else 'off'}  "
              f"proj={'on' if proj_head is not None else 'off'}")
        if proj_head is not None:
            print(f"[Pretrain v6] contrastive: weight={ctr_w_max} τ={ctr_temperature} "
                  f"proj={ctr_proj_hidden}->{ctr_proj_out} warmup={ctr_warmup}ep")

    # ── DDP wrap ────────────────────────────────────────────────────────────
    # All MoRyECGv6FoundationModel parameters are exercised in every forward
    # (no conditional branches that skip whole submodules), so the model can
    # use find_unused_parameters=False — the cheaper backward path. Heads
    # have at most one branch (mlm_head when beat_mask happens to be empty),
    # and apply_masking guarantees ≥1 span per record so even mlm_head is
    # always called in practice.
    fup_model = bool(cfg.get("training", {}).get("ddp_find_unused_model", False))
    fup_head  = bool(cfg.get("training", {}).get("ddp_find_unused_head",  False))
    if ddp:
        model = DDP(model, device_ids=[local_rank], output_device=local_rank,
                    broadcast_buffers=False, find_unused_parameters=fup_model)
        if mlm_head is not None:
            mlm_head = DDP(mlm_head, device_ids=[local_rank], output_device=local_rank,
                           find_unused_parameters=fup_head)
        if rr_head is not None:
            rr_head = DDP(rr_head, device_ids=[local_rank], output_device=local_rank,
                          find_unused_parameters=fup_head)
        if fid_head is not None:
    ...
    raw_mlm_head  = (mlm_head.module  if (ddp and mlm_head  is not None) else mlm_head)
    raw_rr_head   = (rr_head.module   if (ddp and rr_head   is not None) else rr_head)
    raw_fid_head  = (fid_head.module  if (ddp and fid_head  is not None) else fid_head)
    raw_proj_head = (proj_head.module if (ddp and proj_head is not None) else proj_head)

    params = list(model.parameters())
    for h in (mlm_head, rr_head, fid_head, proj_head):
        if h is not None:
            params += list(h.parameters())
    if is_main:
        print(f"[Pretrain v6] Parameters: {sum(p.numel() for p in params):,}  "
              f"(ddp_find_unused_model={fup_model} ddp_find_unused_head={fup_head})")
    ...
    if is_main:
        ...
        print(f"[Pretrain v6] log_dir={log_dir}  tb_dir={tb_dir}")

    mask_token_id = int(cfg["tokenizer"]["codebook_size"])
    morph_w       = float(loss_cfg["morphology_weight"])
    rhythm_w      = float(loss_cfg.get("rhythm_weight", 0.0))     # v5 default 0
    fid_w         = float(loss_cfg.get("fiducial_weight", 0.0))   # v5 default 0
    ...
    if is_main and (rr_mean_t is not None or fid_mean_t is not None):
        print(f"[Pretrain v6] target z-score: rr={'on' if rr_mean_t is not None else 'off'}  "
    ...
    # ── Masking curricula (same schedule helpers as v4) ─────────────────────
    ...
    if is_main:
        print(f"[Pretrain v6] mask_strategy={mask_strategy} span={span_length} "
    ...
    es_bad = 0
    if is_main:
        print(f"[Pretrain v6] early-stop metric={es_metric} "
    ...
    def _heads_state():
        """Pack head state_dicts into the ckpt extra. None heads → None entry."""
        return {
            "mlm_head":  raw_mlm_head.state_dict()  if raw_mlm_head  is not None else None,
            "rr_head":   raw_rr_head.state_dict()   if raw_rr_head   is not None else None,
            "fid_head":  raw_fid_head.state_dict()  if raw_fid_head  is not None else None,
            "proj_head": raw_proj_head.state_dict() if raw_proj_head is not None else None,
        }
    ...
    if resume and os.path.exists(resume):
        ...
        # Skip head loads if the head was disabled in this run (weight=0)
        if raw_mlm_head  is not None and ck.get("mlm_head")  is not None:
            raw_mlm_head.load_state_dict(ck["mlm_head"])
        if raw_rr_head   is not None and ck.get("rr_head")   is not None:
            raw_rr_head.load_state_dict(ck["rr_head"])
        if raw_fid_head  is not None and ck.get("fid_head")  is not None:
            raw_fid_head.load_state_dict(ck["fid_head"])
        if raw_proj_head is not None and ck.get("proj_head") is not None:
        ...
        if is_main:
            print(f"[Resume] {resume}  -> start_epoch={start_epoch}  "
    ...
    if cb_alpha > 0:
        ...
        if os.path.exists(freq_path):
            ...
            if is_main:
                print(f"[Pretrain v6] codebook freq loaded from {freq_path}")
        else:
            if is_main:
                print(f"[Pretrain v6] computing codebook freq on training subset...")
            ...
            if is_main:
                ...
                print(f"[Pretrain v6] codebook freq saved → {freq_path}  "
        ...
        if is_main:
            print(f"[Pretrain v6] class-balanced CE: alpha={cb_alpha}  "
    ...
    # ── Per-view forward (the v5 difference vs v4) ──────────────────────────
    def _compute_view(masked, indices, rr_feats, fid_feats, stft,
                      beat_t_starts, beat_t_ends, beat_valid_mask,
                      B, N, L):
        """One view: build leakage-safe STFT input, run MoRyECG, compute losses.

        AMP policy: model + heads run inside autocast(bf16) for throughput;
        the loss functions (cross_entropy, MSE) run in fp32 outside autocast
        for numerical stability — bf16 has fp32 dynamic range but only 7
        mantissa bits, which is enough for activations but can lose precision
        when reducing across thousands of class logits.
        """
        # ── STFT leakage gate (always exact, no autocast needed for the mask)
        stft_in = apply_stft_leakage_mask(
            stft,
            lead_mask=masked["lead_mask"],
            beat_mask=masked["beat_mask"],
            beat_t_starts=beat_t_starts,
            beat_t_ends=beat_t_ends,
        )

        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
            out = model(masked["masked_indices"], masked["masked_rr_feats"], stft_in,
                        beat_valid_mask=beat_valid_mask)
            H = out["H"]                                                # (B, N, L, D) bf16
            g = out["g"]                                                # (B, D)        bf16

        # MLM only on beat-masked AND valid positions. Padded slots are
        # excluded so the model isn't asked to predict garbage targets.
        beat_mask = masked["beat_mask"] & beat_valid_mask[:, :, None].expand(-1, -1, L)
        if beat_mask.any() and mlm_head is not None:
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                hidden_masked = H[beat_mask]                            # (M, D)
                logits_mlm = mlm_head(hidden_masked)                    # bf16
            # Softmax/CE in fp32 for stability across K=128..2048 classes.
            targets  = indices[beat_mask]
            loss_mlm = F.cross_entropy(logits_mlm.float(), targets, weight=ce_weight)
            with torch.no_grad():
            ...
            if fid_head is not None:
                ...
                loss_fid = F.mse_loss(pred_fid.float(), true_fid)
            else:
        ...
        rr_mask = masked["rhythm_mask"] & beat_valid_mask[:, :, None].expand(-1, -1, L)
        if rr_head is not None and rr_mask.any():
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                pred_rr = rr_head(H[rr_mask])
            true_rr = _normalize_target(rr_feats[rr_mask], rr_mean_t, rr_std_t)
            loss_rr = F.mse_loss(pred_rr.float(), true_rr)
        else:
    ...
    def _contrastive_weight(epoch: int) -> float:
        """Linear warmup: epoch 1 starts at 1/(warmup+1)·max so the gradient
        path through the global token g is *always* live from step 1.
        At epoch == warmup+1, returns full ctr_w_max."""
        if ctr_w_max <= 0:
            return 0.0
        if ctr_warmup <= 0:
            return float(ctr_w_max)
        # epoch is 1-indexed in the train loop. We want:
        #   epoch=1            → ctr_w_max * 1/(warmup+1)  (small but > 0)
        #   epoch=warmup+1     → ctr_w_max                  (full strength)
        # This guarantees that the model's g-path parameters (norm_g, last
        # block's ffn_g/ln_g, proj_head) never receive a strictly-zero
        # gradient — necessary so DDP find_unused_parameters=False is safe
        # from epoch 1 onward.
        if epoch >= ctr_warmup + 1:
            return float(ctr_w_max)
        return float(ctr_w_max) * (epoch / (ctr_warmup + 1))

    # two-view requires the projection head; gate on both
    two_view = (ctr_w_max > 0 and proj_head is not None)
    if is_main and two_view:
        print(f"[Pretrain v6] two-view forward enabled. Initial ctr_w(ep=1)="
              f"{_contrastive_weight(1):.4f} (>0 by design to keep g-path live)")

    # ── Auto-flip find_unused if user disabled the contrastive path ─────────
    # When contrastive is off, the last block's ffn_g / ln_g and norm_g
    # produce gradients only through the contrastive loss. With no contrastive
    # term, those params are unused → DDP errors unless find_unused=True.
    # Detect this and flip the model knob automatically (logged loudly so the
    # user knows; the default v5 config is contrastive-on so this is rare).
    if ddp and not fup_model and (proj_head is None):
        if is_main:
            print("[Pretrain v6] WARNING: proj_head=off but ddp_find_unused_model=False — "
                  "flipping to True so g-path params don't trip DDP unused-param check.")
        # Re-wrap the model with the safer flag.
        model = DDP(model.module, device_ids=[local_rank], output_device=local_rank,
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        cur_lead_dropout = lead_dropout_schedule(
            epoch=epoch, max_prob=ld_max_prob,
            schedule=ld_schedule, warmup_epochs=ld_warmup,
        )
        ...
        if is_main:
            print(f"[Pretrain v6] epoch {epoch}: "
        ...
        for batch in pbar:
            ...
            beat_t_starts  = batch["beat_t_starts"].to(device, non_blocking=True)
            beat_t_ends    = batch["beat_t_ends"].to(device, non_blocking=True)
            beat_valid_mask = batch["beat_valid_mask"].to(device, non_blocking=True)
            ...
            v1 = _compute_view(masked1, indices, rr_feats, fid_feats, stft,
                               beat_t_starts, beat_t_ends, beat_valid_mask, B, N, L)

            if two_view:
                masked2 = _apply_mask(indices, rr_feats, cur_lead_dropout,
                                      cur_beat_ratio, cur_rhythm_ratio)
                v2 = _compute_view(masked2, indices, rr_feats, fid_feats, stft,
                                   beat_t_starts, beat_t_ends, beat_valid_mask, B, N, L)
                ...
                with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                    z1 = proj_head(v1["cls"])
                    z2 = proj_head(v2["cls"])
                # NT-Xent in fp32: the softmax across 2N (= 2 · global_batch)
                # logits is sensitive to small temperature scaling; bf16 here
                # was empirically unstable in early SimCLR papers.
        ...
        if epoch % int(cfg["training"]["eval_every"]) == 0:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    stft_in = apply_stft_leakage_mask(
                        stft, masked["lead_mask"], masked["beat_mask"],
                        beat_t_starts, beat_t_ends,
                    )
                    with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                        out = model(masked["masked_indices"], masked["masked_rr_feats"],
                                    stft_in, beat_valid_mask=beat_valid_mask)
                        H_v = out["H"]                                          # (B, N, L, D)

                    bm = masked["beat_mask"] & beat_valid_mask[:, :, None].expand(-1, -1, L)
                    if bm.any() and mlm_head is not None:
                        hidden_bm = H_v[bm]
                        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                            logits = mlm_head(hidden_bm)
                        tgts   = indices[bm]
                        l_mlm  = F.cross_entropy(logits.float(), tgts, weight=ce_weight).item()
                        ...
                        if fid_head is not None:
                            ...
                            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                                pf = fid_head(hidden_bm)
                            l_fid = F.mse_loss(pf.float(), tgt_fid).item()
                        else:
                            l_fid = 0.0
                    else:
                        l_mlm, accv, accv_nt, l_fid = 0.0, 0.0, 0.0, 0.0

                    rmask = masked["rhythm_mask"] & beat_valid_mask[:, :, None].expand(-1, -1, L)
                    if rr_head is not None and rmask.any():
                        tgt_rr = _normalize_target(rr_feats[rmask], rr_mean_t, rr_std_t)
                        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                            pr = rr_head(H_v[rmask])
                        l_rr = F.mse_loss(pr.float(), tgt_rr).item()
                    else:
                        l_rr = 0.0
        ...
        if es_patience > 0 and evaled:
            ...
            if is_main and es_bad >= es_patience:
                stop_flag = 1
                print(f"[EarlyStop] no {es_metric} improvement for {es_patience} evals; "
    ...
    if is_main:
        print(f"[Pretrain v6] Training complete. "
