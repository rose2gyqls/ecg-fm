"""
Phase 3 (v5 / MoRyECG): Masked beat modeling with physiology-factorized
Transformer + STFT-conditioned [GLOB] + SimCLR contrastive auxiliary loss.

Differences vs training/pretrain/train.py (v4):
  - Model: MoRyECGFoundationModel  (4D H + separate g, intra-beat/inter-beat
  ...
  - ckpt['model_cfg']['arch'] = 'moryecg'  ← used by benchmark adapter

Single GPU:
    python -m training.pretrain.train_v5 --config configs/pretrain/<...>_v5.yaml

Multi-GPU:
    CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 \
        -m training.pretrain.train_v5 --config <...>_v5.yaml
"""
...
from models.transformer.moryecg_model import MoRyECGFoundationModel
...
def train(cfg: dict, resume: str | None = None):
    ...
    if is_main:
        print(f"[Pretrain v5] DDP={ddp}  world_size={world_size}  device={device}  "
    ...
    if is_main:
        print(f"[Pretrain v5] Tokenizer loaded from {tok_ckpt_path} and frozen.")

    # ── Build model_cfg (persisted into ckpt for adapter) ───────────────────
    model_cfg = dict(cfg["model"])
    model_cfg["arch"]             = "moryecg"
    ...
    if rr_mean is not None and rr_std is not None:
        ...
        if is_main:
            print(f"[Pretrain v5] RhythmMLP normalize: mean={ctx['rhythm_mean']} std={ctx['rhythm_std']}")
    ...
    # ── Model + heads ───────────────────────────────────────────────────────
    model = MoRyECGFoundationModel(model_cfg).to(device)
    ...
    if is_main:
        print(f"[Pretrain v5] heads built: "
        ...
        if proj_head is not None:
            print(f"[Pretrain v5] contrastive: weight={ctr_w_max} τ={ctr_temperature} "
                  f"proj={ctr_proj_hidden}->{ctr_proj_out} warmup={ctr_warmup}ep")

    # ── DDP wrap ────────────────────────────────────────────────────────────
    # All MoRyECGFoundationModel parameters are exercised in every forward
    ...
    if is_main:
        print(f"[Pretrain v5] Parameters: {sum(p.numel() for p in params):,}  "
    ...
    if is_main:
        ...
        print(f"[Pretrain v5] log_dir={log_dir}  tb_dir={tb_dir}")
    ...
    if is_main and (rr_mean_t is not None or fid_mean_t is not None):
        print(f"[Pretrain v5] target z-score: rr={'on' if rr_mean_t is not None else 'off'}  "
    ...
    if is_main:
        print(f"[Pretrain v5] mask_strategy={mask_strategy} span={span_length} "
    ...
    es_bad = 0
    if is_main:
        print(f"[Pretrain v5] early-stop metric={es_metric} "
    ...
    if cb_alpha > 0:
        ...
        if os.path.exists(freq_path):
            ...
            if is_main:
                print(f"[Pretrain v5] codebook freq loaded from {freq_path}")
        else:
            if is_main:
                print(f"[Pretrain v5] computing codebook freq on training subset...")
            ...
            if is_main:
                ...
                print(f"[Pretrain v5] codebook freq saved → {freq_path}  "
        ...
        if is_main:
            print(f"[Pretrain v5] class-balanced CE: alpha={cb_alpha}  "
    ...
    if is_main and two_view:
        print(f"[Pretrain v5] two-view forward enabled. Initial ctr_w(ep=1)="
    ...
    if ddp and not fup_model and (proj_head is None):
        if is_main:
            print("[Pretrain v5] WARNING: proj_head=off but ddp_find_unused_model=False — "
    ...
    for epoch in range(start_epoch, max_epochs + 1):
        ...
        if is_main:
            print(f"[Pretrain v5] epoch {epoch}: "
    ...
    if is_main:
        print(f"[Pretrain v5] Training complete. "
