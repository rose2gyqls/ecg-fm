#!/bin/bash
# scripts/run_pretrain_ablation_v2.sh
#
# Pre-training codebook ablation: 512 → 256 → 1024 → 2048 순차 실행.
# 각 codebook별로 ablation tokenizer(checkpoints/tokenizer_heedb_full_cb{N}/best.pt)를
# frozen으로 사용해서 pretrain을 처음부터 학습.
#
# 각 run의 ckpt/log는 분리 저장됨:
#   checkpoints/pretrain_heedb_cb{256,512,1024,2048}/{best,last,epoch_*}.pt
#   logs/pretrain_heedb_cb{256,512,1024,2048}/
#
# Usage:
#   nohup ./scripts/run_pretrain_ablation_v2.sh > pretrain_ablation_v2.log 2>&1 &
#
# Override:
#   GPUS=0,1,2,3,4      ./scripts/run_pretrain_ablation_v2.sh       # GPU 지정 (default: 0~4, 5개)
#   ONLY=cb512          ./scripts/run_pretrain_ablation_v2.sh       # 단일 config만 실행 (쉼표 구분)
#   SKIP=cb256          ./scripts/run_pretrain_ablation_v2.sh       # 특정 config 제외 (쉼표 구분)
#   FORCE_RESTART=1     ./scripts/run_pretrain_ablation_v2.sh       # 완료 stamp 무시 재실행
...
ALL_CONFIGS=(
  "cb512:configs/pretrain/masked_beat_heedb_cb512_v2.yaml"
  "cb256:configs/pretrain/masked_beat_heedb_cb256_v2.yaml"
  "cb1024:configs/pretrain/masked_beat_heedb_cb1024_v2.yaml"
  "cb2048:configs/pretrain/masked_beat_heedb_cb2048_v2.yaml"
)
...
LOG_ROOT="logs/pretrain_ablation_runs_v2"
...
echo "============================================================"
echo "  ECG-FM  |  Pretrain Codebook Ablation v2"
